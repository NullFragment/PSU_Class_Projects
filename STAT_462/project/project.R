...
ggplot(data=melt(baseball[1:13]),aes(x = value)) + facet_wrap(~variable,scales = "free_x") + geom_histogram(bins = 50)
...
select_full = regsubsets(Salary ~ . - Name - Singles,method = "exhaustive",nbest = 1,nvmax=16,data = baseball)
select_red = regsubsets(Salary ~ . - Name - Singles - OBP,method = "exhaustive",nbest = 1,nvmax=16,data = baseball)
