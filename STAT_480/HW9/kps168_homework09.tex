...
\huge STAT 480: Homework 9 \\ % The assignment title
...
%----------------------------------------------------------------------------------------
\newgeometry{top=.75in, bottom=.75in, left=1.25in,right=1.25in}
\section*{\variableheghtrulefill[.25ex]\quad Subject Picture Demo \quad\variableheghtrulefill[.25ex]}
Below is the output for printing the subject using the picture format defined in the code.

\lstinputlisting[language=]{listings/hw9_output.txt}

\section*{\variableheghtrulefill[.25ex]\quad Frequency Count \quad\variableheghtrulefill[.25ex]}
Below is the output for counting the frequencies of all variables other than subject. There is an issue with the states. I could not find where to download the state\_cb file, so I made my own. I assume I'm missing Puerto Rico or an "Other" option. I wasn't sure where to place it as the rest of the states are in alphabetical order.

\lstinputlisting[language=]{listings/hw9_freqs.txt}

\section*{\variableheghtrulefill[.25ex]\quad Format Library Output \quad\variableheghtrulefill[.25ex]}
Below are images of the output from the FMTLIB.

\vspace{1em}

\makebox[\textwidth][c]{\includegraphics[width=.8\textwidth]{format1}}
\makebox[\textwidth][c]{\includegraphics[width=.8\textwidth]{format2}}
\makebox[\textwidth][c]{\includegraphics[width=.8\textwidth]{format3}}

\newpage
\section*{SAS Code}
\lstinputlisting[language=SAS]{hw9.sas}

\newpage
\section*{SAS Log File}
\lstinputlisting[language=]{listings/hw9_logfile.txt}
